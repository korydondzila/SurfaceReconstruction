...
namespace HuguesHoppe
{
	...
	public:
		...
		static FlagMask allocate(int& counter) { assert(counter<max_nflags); return 1u << (counter++); }
	private:
		type _flags{ 0 };
		friend Flag;
	};
	...
	private:
		Flag(Flags& flags, FlagMask fmask) : _flags(flags), _fmask(fmask) { assert(_fmask > 0 && (_fmask&(_fmask - 1)) == 0); }
		Flags& _flags;
